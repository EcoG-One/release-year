...
def _titles_match(query_title: str, candidate_title: str) -> bool:
    ...
    query_title = query_title.replace("’", "'")  # normalize curly apostrophes to straight ones
    candidate_title = candidate_title.replace("’", "'")  # normalize curly apostrophes to straight ones
    return (
...
def _first_year_single(title: str, artist: str) -> int | None:
    ...
    query = f'recording:"{title}" AND artist:"{artist}" AND status:"official" AND type:"single"'
    years = search_recordings()
    if not years:
        # Retry with a more relaxed query if no results were found, in case the
        # recording is not tagged as "official" or "single" for some reason.
